...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        #if no noisy distance, ghost is in jail
        if noisyDistance is None:
            allPossible[self.getJailPosition()] = 1.0
        else:
            #if noisy distance, pacman hasnt came in contact with ghost yet
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        """
        "*** YOUR CODE HERE ***"
        #holds updated beliefs
        newBeliefs = util.Counter()
        for oldPos in self.legalPositions:
            #get the new Position distribution
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            #loop through the position distribution to update beliefs
            for newPos, prob in newPosDist.items():
                #update new belief
                newBeliefs[newPos] = prob * self.beliefs[oldPos] + newBeliefs[newPos]
        #set beliefs to new Beliefs
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        #while particles remain, distribute particles evenly across legal board positions
        while True:
            for pos in self.legalPositions:
                if particlesOnBoard < self.numParticles:
    ...
    def observe(self, observation, gameState):
        ...
        #if noisyDistance is None, ghost is in jail
        if noisyDistance == None:
            self.particles = [self.getJailPosition() for i in range(self.numParticles)]
        else:
            #if noisyDistance is not None, pacman has not come into contact with ghost yet
            for pos in self.legalPositions:
                #find distance from ghost
                trueDistance = util.manhattanDistance(pos, pacmanPosition)
                if emissionModel[trueDistance] > 0:
                    beliefs[pos] = emissionModel[trueDistance] * beliefDist[pos]
            if beliefs.totalCount() == 0:
                #when all particles have zero weight recreate particles
                self.initializeUniformly(gameState)
            else:
    ...
    def elapseTime(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        #sample the position distribution for each position in the particle list
        newBeliefs = []
        for oldPos in self.particles:
            #get new position distribution
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            #sample distribution
            newBelief = util.sample(newPosDist)
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        #store particles as a list
        self.particles = []
        #get the cartesian product of the legal positions and number of ghosts
        ...
        #insert permutation chunks to intialize the particle list
        self.particles = [permutations[i % numPermutations] for i in range(self.numParticles)]
    ...
    def observeState(self, gameState):
        ...
        for particle in range(self.numParticles):
            ...
            for ghost in  range(self.numGhosts):
                #check if a ghost is in jail
                if noisyDistances[ghost] is None:
                    #update particle to be ghost in jail
                    self.particles[particle] = self.getParticleWithGhostInJail(self.particles[particle], ghost)
                else:
                    #when ghosts are not in jail, get manhattan distance
                    distance = util.manhattanDistance(pacmanPosition, self.particles[particle][ghost])
                    particleWeight = particleWeight * (emissionModels[ghost][distance])
            #update distribution with particle weight
            distributions[self.particles[particle]] += particleWeight
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            "*** YOUR CODE HERE ***"
            for i in range(self.numGhosts):
                #place ghosts in old particle positions and find the next position
    ...
    def getBeliefDistribution(self):
        ...
        beliefs = util.Counter()
        for particle in self.particles:
            #add particle weights to the belief distribution
            beliefs[particle] += 1.0
        beliefs.normalize()
        return beliefs
